...
public class InsertArtTest {
	...
	public void C_testErrInsArticolo() throws Exception {
		mockMvc.perform(MockMvcRequestBuilders.post("/api/articoli/inserisci")
				...
				.andExpect(jsonPath("$.messaggio").value("La descrizione deve avere un numero di caratteri compreso tra 6 e 80."))
				.andDo(print());
	}
	...
	public void G_testErrDelArticolo() throws Exception {
		mockMvc.perform(MockMvcRequestBuilders.delete("/api/articoli/elimina/123Test")
				...
				.andExpect(status().isNotFound())
				.andExpect(jsonPath("$.codice").value(404))
				.andExpect(jsonPath("$.messaggio").value("Articolo 123Test non presente in anagrafica! Impossibile utilizzare il metodo DELETE"))
				.andDo(print());
	}
